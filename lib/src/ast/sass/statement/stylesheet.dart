...
final class Stylesheet extends ParentStatement<List<Statement>> {
  ...
  ///
  /// If [parseSelectors] is true, this parses [StyleRule.interpolatedSelector]s
  /// rather than [StyleRule.selector]s.
  ///
  /// Throws a [SassFormatException] if parsing fails.
  factory Stylesheet.parse(String contents, Syntax syntax, {Object? url, bool parseSelectors = false}) {
    try {
      switch (syntax) {
        case Syntax.sass:
          return Stylesheet.parseSass(contents, url: url, parseSelectors: parseSelectors);
        case Syntax.scss:
          return Stylesheet.parseScss(contents, url: url, parseSelectors: parseSelectors);
        case Syntax.css:
          return Stylesheet.parseCss(contents, url: url, parseSelectors: parseSelectors);
      }
  ...
  ///
  /// Throws a [SassFormatException] if parsing fails.
  factory Stylesheet.parseSass(String contents, {Object? url, bool parseSelectors = false}) =>
  ...
  ///
  /// Throws a [SassFormatException] if parsing fails.
  factory Stylesheet.parseScss(String contents, {Object? url, bool parseSelectors = false}) =>
  ...
  ///
  /// Throws a [SassFormatException] if parsing fails.
  factory Stylesheet.parseCss(String contents, {Object? url, bool parseSelectors = false}) =>
