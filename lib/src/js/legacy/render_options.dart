...
extension type RenderOptions._(SharedOptions _) implements SharedOptions {
  ...
  /// options define one.
  ImportCache? get pkgImporterAsyncCache => _pkgImporter
  ...
  /// options.
  NodeImporter importer(DateTime start) => NodeImporter(
      importers.isNotEmpty ? RenderContextOptions(this, start) : JSObject(),
      List<String>.from(includePaths ?? []),
      switch (_importer) {
        null => <JSFunction>[],
        ...
        result.add(
          AsyncCallable.fromSignature(signature.trimLeft(), (arguments) async {
            ...
            var result = wrapJSExceptions(
              () => callback.callAsFunctionVarArgs(context, jsArguments)
                  as JSLegacyValue,
            );
