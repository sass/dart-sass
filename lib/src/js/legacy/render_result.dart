...
@anonymous
extension type RenderResult.__(JSObject _) implements JSObject {
  ...
  factory RenderResult(
    ...
    if (options.enableSourceMaps) {
      ...
      var sourceMapPath = switch (sourceMapOption.asA<JSString>()) {
        var path? => path.toDart,
        _ => options.outFile! + '.map'
      };
      ...
      if (!options.omitSourceMapUrl) {
        var url = options.sourceMapEmbed
            ? Uri.dataFromBytes(sourceMapBytes.toDart, mimeType: "application/json")
            : p.toUri(
                outFile == null
                    ? sourceMapPath
