...
extension type SharedOptions._(JSObject _) implements JSObject {
  ...
  external List<JSAny /*JSDeprecation|String|Version*/ >? get _fatalDeprecations;

  @JS('futureDeprecations')
  external List<JSAny /*JSDeprecation|String*/ >? get _futureDeprecations;

  @JS('silenceDeprecations')
  external List<JSAny /*JSDeprecation|String*/ >? get _silenceDeprecations;

  @JS('logger')
  external JSLogger? get _logger;

  /// Returns the deprecations annotated as fatal by this options object.
List<JSDeprecation> fatalDeprecations(Logger logger) => JSDeprecation.arrayFromJS(
          logger,
          _fatalDeprecations,
...
List<JSDeprecation> futureDeprecations(Logger logger) => JSDeprecation.arrayFromJS(
          logger,
          _fatalDeprecations
        );

  /// Returns the deprecations silenced by this options object.
List<JSDeprecation> silenceDeprecations(Logger logger) => JSDeprecation.arrayFromJS(
          logger,
          _fatalDeprecations
        );
  ...
  Logger logger(Logger fallback, {bool? ascii}) => JSToDartLogger(_logger, fallback, ascii: ascii);
}
