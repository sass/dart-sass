...
extension type JSSassFunction._(JSObject _) implements JSObject {
  ...
  static final JSClass<JSSassFunction> jsClass = () {
    var jsClass = JSClass<JSSassFunction>('sass.SassFunction', (
    JSBoolean self,
    String signature,
    JSFunction callback,
  ) {
    ...
    return SassFunction(
      Callable(
        ...
        (List<Value> values) => callback.callAsFunctionVarArgs(values.cast<JSValue>().toJS),
      ),
    );
  }.toJS);

  SassFunction(Callable('f', '', (_) => sassNull)).toJS.constructor.injectSuperclass(jsClass);
