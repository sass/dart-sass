...
extension type JSSassMap._(JSObject _) implements JSObject {
  ...
  static final JSClass<JSMap> jsClass = () {
    var jsClass = JSClass<JSMap>('sass.SassMap', ((JSMap self, [ImmutableMap? contents]) => contents == null
        ? const SassMap.empty()
        : SassMap(contents.toDart.cast<Value, Value>())).toJS)
    ..defineGetter(
    'contents'.toJS,
    ((JSSassMap self) => self.toDart.contents.cast<JSValue, JSValue>().toJS).toJS,
  )..defineMethod('get'.toJS, (JSSassMap jsSelf, JSAny indexOrKey) {
