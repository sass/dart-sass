...
extension type JSSassList._(JSObject _) implements JSObject {
  ...
  static final JSClass<JSSassList> jsClass = () {
    var jsClass = JSClass('sass.SassList', (
    JSSassList self, [
    ...
    if (ImmutableList.isA(contentsOrOptions)) {
      contents = (contentsOrOptions as ImmutableList<JSValue>).toDart.cast<Value>();
    } else if (contentsOrOptions.isA<JSArray<JSValue>>()) {
      contents = contentsOrOptions.toDart.cast<Value>();
    } else {
      contents = [];
  ...
  }.toJS)..defineMethod('get'.toJS, (JSSassList jsSelf, num indexFloat) {
