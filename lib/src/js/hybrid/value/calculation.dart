...
extension SassCalculationToJS on SassCalculation {
  ...
  static final JSClass<UnsafeDartWrapper<SassCalculation>> jsClass = () {
    ...
    // interop type.
    void constructor(UnsafeDartWrapper<SassCalculation> self, [JSAny? _]) {
      JSError.throwLikeJS(JSError("new sass.SassCalculation() isn't allowed."));
    }

    var jsClass = JSClass<UnsafeDartWrapper<SassCalculation>>(
        'sass.SassCalculation', constructor.toJS)
      ..defineStaticMethods({
      ...
      ..defineGetters({
        'name': (UnsafeDartWrapper<SassCalculation> self) =>
            self.toDart.name.toJS,
        'arguments': (UnsafeDartWrapper<SassCalculation> self) => self
            .toDart.arguments
...
extension CalculationOperationToJS on CalculationOperation {
  ...
  static final JSClass<UnsafeDartWrapper<CalculationOperation>> jsClass = () {
    var jsClass = JSClass<UnsafeDartWrapper<CalculationOperation>>(
        ...
        (UnsafeDartWrapper<CalculationOperation> self, String strOperator,
            JSAny left, JSAny right) {
...
extension CalculationInterpolationToJS on CalculationInterpolation {
  ...
  static final JSClass<UnsafeDartWrapper<CalculationInterpolation>> jsClass =
    ...
    var jsClass = JSClass<UnsafeDartWrapper<CalculationInterpolation>>(
        ...
        ((UnsafeDartWrapper<CalculationInterpolation> self, String value) =>
            CalculationInterpolation(value).toJS).toJS)
      ..defineMethods({
        'equals':
            ((UnsafeDartWrapper<CalculationInterpolation> self, JSAny? other) =>
                switch (other.asClassOrNull(
                    CalculationInterpolationToJS.jsClass)) {
