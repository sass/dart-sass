...
extension type JSSassCalculation._(JSObject _) implements JSObject {
  ...
  static final JSClass<JSSassCalculation> jsClass = () {
    var jsClass = JSClass<JSSassCalculation>(
        'sass.SassCalculation',
        (JSSassCalculation self, [JSAny? _]) {
          JSError.throwLikeJS(
              JSError("new sass.SassCalculation() isn't allowed."));
        }.toJS)
      ..defineStaticMethods({
        ...
        'clamp': (JSObject min, [JSObject? value, JSObject? max]) {
          ...
          return SassCalculation.unsimplified(
                  'clamp', [min, value, max].nonNulls)
              .toJS;
        }.toJS,
      })
      ..defineMethod(
        'assertCalculation'.toJS,
        ((JSSassCalculation self, [String? name]) => self).toJS,
      )
      ..defineGetters({
        'name': (JSSassCalculation self) => self.toDart.name.toJS,
        'arguments': (JSSassCalculation self) =>
            self.toDart.arguments.cast<JSValue>().toJSImmutable,
      });
...
extension type JSCalculationOperation._(JSObject _) implements JSObject {
  ...
  static final JSClass<JSCalculationOperation> jsClass = () {
    var jsClass = JSClass<JSCalculationOperation>(
        'sass.CalculationOperation',
        (JSCalculationOperation self, String strOperator, JSObject left,
            JSObject right) {
      ...
      ..defineMethods({
        'equals': ((JSCalculationOperation self, JSAny? other) =>
            self.toDart == other).toJS,
        'hashCode':
            ((JSCalculationOperation self) => self.toDart.hashCode).toJS,
      })
      ..defineGetters({
        'operator': (JSCalculationOperation self) =>
            self.toDart.operator.operator.toJS,
        'left': (JSCalculationOperation self) => self.toDart.left as JSObject,
        'right': (JSCalculationOperation self) => self.toDart.right as JSObject,
      });
  ...
  CalculationOperation get toDart => this as CalculationOperation;
}
...
extension type JSCalculationInterpolation._(JSObject _) implements JSObject {
  ...
  static final JSClass<JSCalculationInterpolation> jsClass = () {
    var jsClass = JSClass<JSCalculationInterpolation>(
        'sass.CalculationInterpolation',
        ((JSCalculationInterpolation self, String value) =>
            CalculationInterpolation(value)).toJS)
      ..defineMethods({
        'equals': ((JSCalculationInterpolation self, JSAny? other) =>
            self.toDart == other).toJS,
        'hashCode':
            ((JSCalculationInterpolation self) => self.toDart.hashCode).toJS,
      })
      ..defineGetter('value'.toJS,
          (JSCalculationInterpolation self) => self.toDart.value.toJS);
  ...
  CalculationInterpolation get toDart => this as CalculationInterpolation;
}
