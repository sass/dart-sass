...
extension type JSValue._(JSObject _) implements JSObject {
  ...
  static final JSClass<JSValue> jsClass =
    ...
    ..defineGetters({
    'asList': ((JSValue self) => self.toDart.asList.cast<JSValue>().toJSImmutable).toJS,
    ...
    'sassIndexToListIndex': ((JSValue self, JSValue sassIndex, [String? name]) =>
        self.toDart.sassIndexToListIndex(sassIndex.toDart, name)).toJS,
    'get': ((JSValue self, num index) =>
        index < 1 && index >= -1 ? self : undefined).toJS,
    'assertBoolean': ((JSValue self, [String? name]) => self.toDart.assertBoolean(name).toJS).toJS,
    'assertCalculation': ((JSValue self, [String? name]) =>
        self.toDart.assertCalculation(name).toJS).toJS,
    'assertColor': ((JSValue self, [String? name]) => self.toDart.assertColor(name).toJS).toJS,
    'assertFunction': ((JSValue self, [String? name]) => self.toDart.assertFunction(name).toJS).toJS,
    'assertMap': ((JSValue self, [String? name]) => self.toDart.assertMap(name).toJS).toJS,
    'assertMixin': ((JSValue self, [String? name]) => self.toDart.assertMixin(name).toJS).toJS,
    'assertNumber': ((JSValue self, [String? name]) => self.toDart.assertNumber(name).toJS).toJS,
    'assertString': ((JSValue self, [String? name]) => self.toDart.assertString(name).toJS).toJS,
