...
/// object.
@anonymous
extension type JSSassException.__(JSError _) implements JSError {
  static final JSClass<JSSassException> jsClass = () {
    ...
    var jsClass = JSClass<JSSassException>.extend('sass.Exception', _jsErrorClass,
        (thisThunk, superclassConstructor, args) {
      var dartException =
          (args[0] as ExternalDartReference<SassException>).toDartObject;
      var message = (args[1] as JSString).toDart;
      superclassConstructor.callAsFunction(args[0]);
      ...
      thisThunk.thisArg.defineProperty(
          ...
          JSPropertyDescriptor.getValue(
              dartException.toExternalReference as JSAny, enumerable: false));
    });
    ...
    jsClass.defineGetters({
      'sassMessage': (JSSassException exception) =>
          exception.dartException.toDartObject.message.toJS,
      'sassStack': (JSSassException exception) =>
          exception.dartException.toDartObject.trace.toString().toJS,
      'span': (JSSassException exception) => exception.dartException.toDartObject.span.toJS,
    });
  ...
  factory JSSassException(
    ...
    glyph.ascii = ascii;
    try {
      var jsException = JSSassException._(
        exception.toExternalReference,
        exception.toString(color: color).replaceFirst('Error: ', ''),
      );
      trace = getTrace(exception) ?? trace;
      if (trace != null) jsException.stack = trace.toString();
      return jsException;
    } finally {
  ...
  external JSSassException._(ExternalDartReference<SassException> dartException, String message);

  external ExternalDartReference<SassException> get dartException;
}
