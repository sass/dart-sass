...
class ExtensionStore {
  ...
  List<ComplexSelector>? _extendComplex(
    ...
    for (var i = 0; i < complex.components.length; i++) {
      ...
      if (extended == null) {
        extendedNotExpanded?.add([
          ComplexSelector([component], complex.span,
        ...
        extendedNotExpanded = [
          [
            ComplexSelector(complex.components.take(i), complex.span,
          ...
          [
            for (var newComplex in extended)
              if (newComplex.leadingCombinator == null ||
                ...
                ComplexSelector(newComplex.components, complex.span,
                    leadingCombinator: complex.leadingCombinator,
    ...
    var first = true;
    var result = paths(extendedNotExpanded).expand((path) {
  ...
  List<ComplexSelector>? _unifyExtenders(List<Extender> extenders,
    ...
    if (originals != null) {
      toUnify.addFirst(ComplexSelector([
  ...
  Extender _extenderForSimple(SimpleSelector simple) => Extender(
      ComplexSelector([
        ComplexSelectorComponent(
            CompoundSelector([simple], simple.span), simple.span)
      ], simple.span),
