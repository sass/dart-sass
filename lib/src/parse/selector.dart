...
class SelectorParser extends Parser {
  ...
  SelectorList _selectorList({bool allowLeadingCombinator = true}) {
    var start = scanner.state;
    var previousLine = scanner.line;
    var components = <ComplexSelector>[
      _complexSelector(allowLeadingCombinator: allowLeadingCombinator)
    ];
  ...
  ///
  /// If [allowLeadingCombinator] is false, leading combinators will be syntax
  /// errors.
  ///
  /// If [lineBreak] is `true`, that indicates that there was a line break
  /// before this selector.
  ComplexSelector _complexSelector(
      {bool allowLeadingCombinator = true, bool lineBreak = false}) {
    var start = scanner.state;
  ...
  PseudoSelector _pseudoSelector() {
    ...
    if (element) {
      if (_selectorPseudoElements.contains(unvendored)) {
        selector = _selectorList(allowLeadingCombinator: false);
      } else {
        argument = declarationValue(allowEmpty: true);
      }
    } else if (_selectorPseudoClasses.contains(unvendored)) {
      selector = _selectorList(allowLeadingCombinator: name == 'has');
