...
abstract class StylesheetParser extends Parser {
  ...
  StyleRule _styleRule([
    ...
    if (_parseSelectors) {
      ...
      return _withStyleRuleChildren(
          ...
          start,
          (children, span) =>
              StyleRule.interpolated(selector, children, spanFrom(start)));
    } else {
  ...
  ///
  /// If [allowOf] is `false`, this stops at a top-level identifier with the
  /// value "of".
  ///
    ...
    while (true) {
      switch (scanner.peekChar()) {
        ...
        case $o || $O:
          if (!allowOf &&
              brackets.isEmpty &&
              scanIdentifier("of", caseSensitive: false)) {
            break loop;
          }
          buffer.writeCharCode(scanner.readChar());
          wroteNewline = false;
  ...
  Interpolation _interpolatedIdentifierBody() {
    ...
    return text.interpolation(scanner.spanFrom(start));
  }
  ...
  InterpolatedAttributeSelector _attributeSelector() {
    ...
    var modifier = lookingAtIdentifier() ? interpolatedIdentifier() : null;
    whitespace(consumeNewlines: true);
  ...
  InterpolatedQualifiedName _attributeName() {
    ...
    if (scanner.scanChar($asterisk)) {
      var namespace = Interpolation.plain("*", scanner.spanFrom(start));
    ...
    if (scanner.scanChar($pipe)) {
      var namespace = Interpolation.plain("", scanner.spanFrom(start, start));
      return InterpolatedQualifiedName(
          interpolatedIdentifier(), spanFrom(start),
          namespace: namespace);
    }
  ...
  InterpolatedParentSelector _parentSelector() {
    ...
    scanner.expectChar($ampersand);
    var suffix =
        lookingAtIdentifierBody() ? _interpolatedIdentifierBody() : null;
  ...
  InterpolatedPseudoSelector _pseudoSelector() {
    ...
    if (element) {
      ...
      argument =
          _interpolatedDeclarationValue(allowOf: false, consumeNewlines: true);
      whitespace(consumeNewlines: true);
  ...
  InterpolatedSimpleSelector _typeOrUniversalSelector() {
    ...
    if (scanner.scanChar($asterisk)) {
      ...
      var namespace =
          Interpolation.plain("*", scanner.spanFrom(start, afterAsterisk));
