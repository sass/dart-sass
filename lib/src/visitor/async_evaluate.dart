...
final class _EvaluateVisitor({
  ...
  final AsyncImportCache _importCache = importCache ?? AsyncImportCache.none();
  ...
  AsyncEnvironment _environment = AsyncEnvironment();
      ...
      if (canonicalUrl != null) {
        if (_activeModules.containsKey(canonicalUrl)) {
          ...
          throw _activeModules[canonicalUrl].andThen(
                (previousLoad) => _multiSpanException(message, "new load", {
  ...
  void _extendModules(Iterable<Module> sortedModules) {
    ...
    for (var module in sortedModules) {
      ...
      // already.
      unsatisfiedExtensions.addAll(
        module.extensionStore.extensionsWhereTarget(
          (target) => !originalSelectors.contains(target),
        ),
      );

      downstreamExtensionStores[module.url].andThen(
      ...
      unsatisfiedExtensions.removeAll(
        module.extensionStore.extensionsWhereTarget(originalSelectors.contains),
      );
    }
  ...
  Future<Value?> visitAtRootRule(AtRootRule node) async {
    ...
    if (node.query case var unparsedQuery?) {
      var (resolved, map) = await _performInterpolationWithMap(
        unparsedQuery,
        warnForColor: true,
      );
  ...
  Future<Value?> visitExtendRule(ExtendRule node) async {
    ...
    var (targetText, targetMap) = await _performInterpolationWithMap(
      node.selector,
      warnForColor: true,
    );

    var list = SelectorList.parse(
  ...
  Future<Value?> visitAtRule(AtRule node) async {
    ...
    var value = await node.value.andThen(
      (value) => _interpolationToValue(value, trim: true, warnForColor: true),
    );
  ...
  Future<Value?> visitForRule(ForRule node) async {
    ...
    var toNumber = await _addExceptionSpanAsync(
      node.to,
      () async => (await node.to.accept(this)).assertNumber(),
    );
  ...
  Future<Value?> visitForwardRule(ForwardRule node) async {
    ...
    if (node.configuration.isNotEmpty) {
      var newConfiguration = await _addForwardConfiguration(
  ...
  void _assertConfigurationIsEmpty(
    Configuration configuration, {
  ...
  Future<Value?> visitIfRule(IfRule node) async {
    ...
    return await clause.andThen<Future<Value?>>(
      (clause) => _environment.scope(
    ...
    return _withStackFrame("@import", import, () async {
      ...
      if (url != null) {
        if (_activeModules.containsKey(url)) {
          throw _activeModules[url].andThen(
                (previousLoad) => _multiSpanException(
  ...
  /// This first tries loading [url] relative to [baseUrl], which defaults to
  /// `_stylesheet.span.sourceUrl`.
  Future<_LoadedStylesheet> _loadStylesheet(
    String url,
    FileSpan span, {
    Uri? baseUrl,
    ...
    try {
      ...
      if (_importCache case var importCache) {
        ...
        if (await importCache.canonicalize(
              Uri.parse(url),
          ...
          if (await importCache.importCanonical(
                importer,
                canonicalUrl,
  ...
  Future<void> _visitStaticImport(StaticImport import) async {
    ...
    var node = ModifiableCssImport(
      await _interpolationToValue(import.url),
      import.span,
      modifiers: await import.modifiers.andThen<Future<CssValue<String>>?>(
  ...
  Future<Value?> visitIncludeRule(IncludeRule node) async {
    ...
    var nodeWithSpanWithoutContent = AstNode.fake(
  ...
  List<CssMediaQuery>? _mergeMediaQueries(
    Iterable<CssMediaQuery> queries1,
    Iterable<CssMediaQuery> queries2,
  ) {
  ...
  Future<Value?> visitStyleRule(StyleRule node) async {
    ...
    if (_inKeyframes) {
      ...
      var parsedSelector = KeyframeSelectorParser(
        selectorText,
    ...
    var parsedSelector = SelectorList.parse(
      selectorText,
      interpolationMap: selectorMap,
    ...
    var rule = ModifiableCssStyleRule(
      ...
      fromPlainCss: _stylesheet.plainCss,
    );
    var oldAtRootExcludingStyleRule = _atRootExcludingStyleRule;
    _atRootExcludingStyleRule = false;
    await _withParent(
    ...
    for (var complex in rule.selector.components) {
      ...
      if (complex.leadingCombinator != null) {
        ...
        throw MultiSpanSassRuntimeException(
          ...
          'invalid selector',
          {
            rule.children.first.span:
                "this is not a style rule${rule.children.every((child) => child is CssComment) ? '\n(try converting to a //-style comment)' : ''}",
          },
          _stackTrace(complex.span),
        );
      }
  ...
  Future<String> _visitSupportsCondition(SupportsCondition condition) async =>
      switch (condition) {
        ...
        SupportsInterpolation interpolation => await _evaluateToCss(
          interpolation.expression,
          quote: false,
        ),
        SupportsDeclaration declaration => await _withSupportsDeclaration(
          () async =>
              "(${await _evaluateToCss(declaration.name)}:"
  ...
  Future<Value> visitBinaryOperationExpression(BinaryOperationExpression node) {
    if (_stylesheet.plainCss && node.operator != .singleEquals) {
      throw _exception(
    ...
    return _addExceptionSpanAsync(node, () async {
      ...
      return switch (node.operator) {
        .singleEquals => left.singleEquals(await node.right.accept(this)),
        .or => left.isTruthy ? left : await node.right.accept(this),
        .and => left.isTruthy ? await node.right.accept(this) : left,
        .equals => SassBoolean(left == await node.right.accept(this)),
        .notEquals => SassBoolean(left != await node.right.accept(this)),
        .greaterThan => left.greaterThan(await node.right.accept(this)),
        .greaterThanOrEquals => left.greaterThanOrEquals(
          await node.right.accept(this),
        ),
        .lessThan => left.lessThan(await node.right.accept(this)),
        .lessThanOrEquals => left.lessThanOrEquals(
          await node.right.accept(this),
        ),
        .plus => left.plus(await node.right.accept(this)),
        .minus => left.minus(await node.right.accept(this)),
        .times => left.times(await node.right.accept(this)),
        .modulo => left.modulo(await node.right.accept(this)),
        ...
        /// it here too.
        .dividedBy => left.dividedBy(await node.right.accept(this)),
      };
    });
  }
  ...
  ) async => switch (await node.expression.accept(this)) {
  ...
  Future<Object /* String | bool */> visitIfConditionFunction(
    IfConditionFunction node,
      ...
      '${await _performInterpolation(node.name)}(${await _performInterpolation(node.arguments)})';

  @override
  Future<Object /* String | bool */> visitIfConditionSass(
    IfConditionSass node,
  ) async => (await node.expression.accept(this)).isTruthy;

  @override
  Future<Object /* String | bool */> visitIfConditionRaw(
    IfConditionRaw node,
  ) async => await _performInterpolation(node.text);
  ...
  Future<Value> visitFunctionExpression(FunctionExpression node) async {
      ...
      switch (node.name.toLowerCase()) {
        case ("min" || "max" || "round" || "abs") && var name
            when node.arguments.named.isEmpty &&
                ...
                node.arguments.positional.every(
                  (argument) => argument.isCalculationSafe,
                ):
    ...
    var result = await _addErrorSpan(
      node,
      () => _runFunctionCallable(node.arguments, function, node),
    );
  ...
  Future<Value> _visitCalculation(
    ...
    var arguments = [
      for (var argument in node.arguments.positional)
        await _visitCalculationExpression(
          argument,
  ...
  void _checkCalculationArguments(FunctionExpression node) {
    void check([int? maxArgs]) {
      if (node.arguments.positional.isEmpty) {
        ...
        throw _exception(
          "Only $maxArgs ${pluralize('argument', maxArgs)} allowed, but ${node.arguments.positional.length} ${pluralize('was', node.arguments.positional.length, plural: 'were')} passed.",
          node.span,
        );
      }
    }
  ...
  /// The parameter is the name of the function, which is used for reporting
  /// deprecation warnings.
  Future<Object> _visitCalculationExpression(
    Expression node, {
    required String? inLegacySassFunction,
  }) async {
    switch (node) {
      case ParenthesizedExpression(expression: var inner):
        var result = await _visitCalculationExpression(
          inner,
          inLegacySassFunction: inLegacySassFunction,
        );
      ...
      case ListExpression(
        ...
        var adjusted = _inFunction || _inMixin
            ...
            : _adjustSlashPrecedence(contents);
        return await _visitCalculationExpression(
          adjusted,
          inLegacySassFunction: inLegacySassFunction,
        );
  ...
  Expression _adjustSlashPrecedence(List<Expression> contents) {
    ...
    for (var right in contents.skip(1)) {
      if (right case StringExpression(
      ...
      left = switch ((
        ...
        (
          ...
          BinaryOperationExpression(
            ...
            BinaryOperationExpression(
              ...
              leftRemainder,
              // 2 / 3
              BinaryOperationExpression(.dividedBy, leftOperand, rightOperand),
            ),
            rightRemainder,
          ),
        // Example: 1 + 2 / 3
        ...
        ((var leftRemainder, var leftOperator, var leftOperand), _) =>
          ...
          BinaryOperationExpression(
            ...
            leftRemainder,
            // 2 / 3
            BinaryOperationExpression(.dividedBy, leftOperand, right),
          ),
        // Example: 1 / 2 + 3
        //       2                 +                  3
        (_, (var rightOperand, var rightOperator, var rightRemainder)) =>
          ...
          BinaryOperationExpression(
            rightOperator,
            // 1 / 2
            BinaryOperationExpression(.dividedBy, left, rightOperand),
            rightRemainder,
          ),
        _ => switch (right) {
          // Example: 1 / 2 * 3
          ...
          _ => BinaryOperationExpression(.dividedBy, left, right),
        },
      };
    }
  ...
  (Expression, BinaryOperator, Expression)? _splitCalculationSumHead(
    ...
    switch (expression) {
      ...
      case BinaryOperationExpression(
        // -
        operator: (.plus || .minus) && var operator,
        left: BinaryOperationExpression(operator: .plus || .minus) &&
            var left, // ((1 + 2) + 3)
        :var right, // 4
      ):
        // (1, +, 2 + 3)
        var (head, leftOperator, leftRemainder) = _splitCalculationSumHead(
          left,
        )!;
        // (1, +, (2 + 3) - 4)
        return (
  ...
  (Expression, BinaryOperator, Expression)? _splitCalculationSumTail(
    ...
    switch (expression) {
      case BinaryOperationExpression(
        ...
        operator: (.plus || .minus) && var operator,
        :var left, // 1
        right: BinaryOperationExpression(operator: .plus || .minus) &&
            var right, // 2 + (3 + 4)
      ):
        // (2 + 3, +, 4)
        var (rightRemainder, rightOperator, tail) = _splitCalculationSumTail(
          right,
        )!;
        // (1 - (2 + 3), + 4)
        return (
  ...
  void _checkWhitespaceAroundCalculationOperator(
    ...
    var textBetweenOperands = node.left.span.file.getText(
    ...
    var result = await _withStackFrame(name, nodeWithSpan, () {
      ...
      return _withEnvironment(callable.environment.closure(), () {
        return _environment.scope(() async {
          ...
          var parameterWord = pluralize(
            'parameter',
            evaluated.named.keys.length,
          );
          var parameterNames = toSentence(
            evaluated.named.keys.map((name) => "\$$name"),
            'or',
          );
          throw MultiSpanSassRuntimeException(
  ...
  Future<Value> _runFunctionCallable(
    ArgumentList arguments,
  ...
  /// body.
  Future<Value> _runBuiltInCallable(
    ArgumentList arguments,
    AsyncBuiltInCallable callable,
    AstNode nodeWithSpan,
  ) async {
    var evaluated = await _evaluateArguments(arguments);
    ...
    if (overload.restParameter != null) {
      ...
      if (evaluated.positional.length > parameters.length) {
        ...
        evaluated.positional.removeRange(
          parameters.length,
          evaluated.positional.length,
        );
      }
    ...
    try {
      ...
      throwWithTrace(
        _exception(_getErrorMessage(error), nodeWithSpan.span),
        error,
        stackTrace,
      );
    }
  ...
  Future<SassString> visitSupportsExpression(
    ...
    await _visitSupportsCondition(expression.condition),
    quotes: false,
  );
  ...
  Future<void> visitCssImport(CssImport node) async {
    ...
    var modifiableNode = ModifiableCssImport(
      node.url,
      node.span,
  ...
  Future<void> visitCssMediaRule(CssMediaRule node) async {
    ...
    if (_hasCssNesting) {
      await _withParent(
        ModifiableCssMediaRule(node.queries, node.span),
        () async {
          for (var child in node.children) {
            await child.accept(this);
          }
  ...
  Future<CssValue<String>> _interpolationToValue(
    Interpolation interpolation, {
    bool trim = false,
    bool warnForColor = false,
  }) async {
    var result = await _performInterpolation(
      interpolation,
      warnForColor: warnForColor,
    );
    return CssValue(
      trim ? trimAscii(result, excludeEscape: true) : result,
      interpolation.span,
    );
  }
  ...
  /// values passed into the interpolation.
  Future<String> _performInterpolation(
    Interpolation interpolation, {
    bool warnForColor = false,
  }) async {
    var (result, _) = await _performInterpolationHelper(
      interpolation,
      sourceMap: false,
      warnForColor: warnForColor,
    );
  ...
  Future<(String, InterpolationMap)> _performInterpolationWithMap(
    ...
    var (result, map) = await _performInterpolationHelper(
      interpolation,
      sourceMap: true,
      warnForColor: warnForColor,
    );
    ...
    for (var value in interpolation.contents) {
      ...
      if (warnForColor && namesByColor.containsKey(result)) {
        var alternative = BinaryOperationExpression(
          .plus,
          StringExpression(.plain("", interpolation.span), quotes: true),
          expression,
        );
        _warn(
  ...
  SassRuntimeException _multiSpanException(
    String message,
    String primaryLabel,
  ...
  T _addExceptionSpan<T>(
    ...
    try {
      ...
      throwWithTrace(
        _adjustException(error, nodeWithSpan.span, label: label),
        error,
        stackTrace,
      );
    }
  ...
  Future<T> _addExceptionTrace<T>(FutureOr<T> Function() callback) async {
    try {
      ...
      throwWithTrace(
        error.withTrace(_stackTrace(error.span)),
        error,
        stackTrace,
      );
    }
