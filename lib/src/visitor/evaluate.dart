...
//
// Checksum: 4c664d5dc9296b497c53de4d22f17b48e277b039
//
...
final class _EvaluateVisitor({
  ...
  Value? visitExtendRule(ExtendRule node) {
    ...
    var (targetText, targetMap) = _performInterpolationWithMap(
      node.selector,
      warnForColor: true,
    );
  ...
  Value? visitForRule(ForRule node) {
    var fromNumber = _addExceptionSpan(
      node.from,
      () => node.from.accept(this).assertNumber(),
    );
    var toNumber = _addExceptionSpan(
      node.to,
      () => node.to.accept(this).assertNumber(),
    );
  ...
  Value? visitForwardRule(ForwardRule node) {
    ...
    if (node.configuration.isNotEmpty) {
      var newConfiguration = _addForwardConfiguration(
  ...
  /// This first tries loading [url] relative to [baseUrl], which defaults to
  /// `_stylesheet.span.sourceUrl`.
  _LoadedStylesheet _loadStylesheet(
    String url,
    FileSpan span, {
    Uri? baseUrl,
  ...
  void _visitStaticImport(StaticImport import) {
    ...
    var node = ModifiableCssImport(
      _interpolationToValue(import.url),
      import.span,
      modifiers: import.modifiers.andThen<CssValue<String>?>(
  ...
  Value? visitMediaRule(MediaRule node) {
    ...
    // that support nesting natively anyway.
    if (_hasCssNesting) {
      _withParent(ModifiableCssMediaRule(queries, node.span), () {
        for (var child in node.children) {
          child.accept(this);
        }
      }, scopeWhen: false);
  ...
  Value visitBinaryOperationExpression(BinaryOperationExpression node) {
    ...
    return _addExceptionSpan(node, () {
      ...
      return switch (node.operator) {
        ...
        /// it here too.
        .dividedBy => left.dividedBy(node.right.accept(this)),
      };
    });
  }
  ...
  Object /* String | bool */ visitIfConditionFunction(
    IfConditionFunction node,
      ...
      '${_performInterpolation(node.name)}(${_performInterpolation(node.arguments)})';
    ...
    switch (node) {
      ...
      case ListExpression(
        ...
        var adjusted = _inFunction || _inMixin
            ...
            : _adjustSlashPrecedence(contents);
        return _visitCalculationExpression(
          adjusted,
          inLegacySassFunction: inLegacySassFunction,
        );
  ...
  void visitCssMediaRule(CssMediaRule node) {
    ...
    // that support nesting natively anyway.
    if (_hasCssNesting) {
      _withParent(ModifiableCssMediaRule(node.queries, node.span), () {
        for (var child in node.children) {
          child.accept(this);
        }
      }, scopeWhen: false);
  ...
  CssValue<String> _interpolationToValue(
    ...
    var result = _performInterpolation(
      interpolation,
  ...
  /// values passed into the interpolation.
  String _performInterpolation(
    Interpolation interpolation, {
    bool warnForColor = false,
  }) {
    var (result, _) = _performInterpolationHelper(
      interpolation,
      sourceMap: false,
      warnForColor: warnForColor,
    );
  ...
  (String, InterpolationMap) _performInterpolationWithMap(
    ...
    var (result, map) = _performInterpolationHelper(
      interpolation,
      sourceMap: true,
      warnForColor: warnForColor,
    );
