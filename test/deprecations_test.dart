...
void main() {
  ...
  group("colorModuleCompat is violated by", () {
    ...
    test("passing a number to color.invert", () {
      _expectDeprecation("$color.invert(0)}", .colorModuleCompat);
    });

    test("passing a number to color.grayscale", () {
      _expectDeprecation("$color.grayscale(0)}", .colorModuleCompat);
    });

    test("passing a number to color.opacity", () {
      _expectDeprecation("$color.opacity(0)}", .colorModuleCompat);
    });

    test("using color.alpha for a microsoft filter", () {
      _expectDeprecation("$color.alpha(foo=bar)}", .colorModuleCompat);
    });
  });

  // Deprecated in 1.33.0
  test("slashDiv is violated by using / for division", () {
    _expectDeprecation(r"a {b: (4/2)}", .slashDiv);
  });

  // Deprecated in 1.54.0
  group("bogusCombinators is violated by", () {
    test("adjacent combinators", () {
      _expectDeprecation("a > > a {b: c}", .bogusCombinators);
    });

    test("leading combinators", () {
      _expectDeprecation("a > {b: c}", .bogusCombinators);
    });
  ...
  group("strictUnary is violated by", () {
    test("an ambiguous + operator", () {
      _expectDeprecation(r"a {b: 1 +2}", .strictUnary);
    });

    test("an ambiguous - operator", () {
      _expectDeprecation(r"a {$x: 2; b: 1 -$x}", .strictUnary);
    });
  });
    ...
    test("a fully relative URL", () {
      _expectDeprecationCallback(
        () => compileStringToResult(
          "a {b: c}",
          url: "foo",
          fatalDeprecations: {.compileStringRelativeUrl},
        ),
  ...
  group("functionUnits is violated by", () {
    test("a hue with a non-angle unit", () {
      _expectDeprecation("a {b: hsl(10px, 0%, 0%)}", .functionUnits);
    });

    test("a saturation/lightness with a non-percent unit", () {
      _expectDeprecation("a {b: hsl(10deg, 0px, 0%)}", .functionUnits);
    });

    test("a saturation/lightness with no unit", () {
      _expectDeprecation("a {b: hsl(10deg, 0%, 0)}", .functionUnits);
    });
