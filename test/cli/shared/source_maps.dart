...
void sharedTests(Future<TestProcess> runSass(Iterable<String> arguments)) {
  ...
  group("with multiple sources", () {
    ...
    test(
      "refers to them using relative URLs with --source-map-urls=relative",
      () async {
        await (await runSass([
    ...
    test(
      "refers to them using absolute URLs with --source-map-urls=absolute",
      () async {
        await (await runSass([
  ...
  group("when emitting to stdout", () {
    test("--source-map isn't allowed", () async {
      ...
      expect(
        sass.stdout,
        emits(
          "When printing to stdout, --source-map requires "
          "--embed-source-map.",
        ),
      );
      expect(
    ...
    test(
      ...
      expect(
        sass.stdout,
        emits(
          "--source-map-urls=relative isn't allowed when printing to "
          "stdout.",
        ),
      );
      expect(
