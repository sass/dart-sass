...
describe('a complex selector', () => {
  ...
  describe('stringifies', () => {
    describe('with one child', () => {
      it('with no raws', () =>
        expect(new SelectorList({class: 'foo'}).toString()).toBe('.foo'));
      ...
      it('ignores extra complex raws', () =>
        expect(
          new SelectorList({
            class: 'foo',
            raws: {compounds: [undefined, {before: '  ', after: '/**/'}]},
          }).toString(),
        ).toBe('.foo'));
    });
    ...
    it('with multiple children', () => {
      it('with no raws', () =>
        expect(
          new SelectorList([{class: 'foo'}, {class: 'bar'}]).toString(),
        ).toBe('.foo, .bar'));
        ...
        describe('at the end', () => {
          it('before', () =>
            expect(
              new SelectorList({
                nodes: [{class: 'foo'}, {class: 'bar'}],
                raws: {compound: [undefined, {before: '  '}]},
              }).toString(),
            ).toBe('.foo,  .bar'));
          ...
          it('both', () =>
            expect(
              new SelectorList({
                nodes: [{class: 'foo'}, {class: 'bar'}],
                raws: {compound: [undefined, {before: '  ', after: '/**/'}]},
              }).toString(),
            ).toBe('.foo,  .bar/**/'));
        });
