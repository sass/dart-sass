...
describe('a class selector', () => {
  ...
  describe('with an escaped class', () => {
    function describeNode(
      description: string,
      create: () => ClassSelector,
    ): void {
      describe(description, () => {
        ...
        it('has a class', () =>
          expect(node).toHaveInterpolation('class', 'f\\0ao'));
      });
    }

    // Specifically, this is an escape that's more verbose than it needs to be.
    // We want to verify that we aren't parsing the class semantically.
    describeNode('parsed', () => parseSimpleSelector('.f\\0aoo'));
