...
export class Interpolation extends Node {
   ...
   *
   * This method only iterates through the interpolation’s immediate children.
  ...
  /**
   * Inserts {@link newNode} immediately after the first occurance of {@link
   * oldNode} in {@link nodes}.
   *
  ...
  /**
   * Inserts {@link newNode} immediately before the first occurance of {@link
   * oldNode} in {@link nodes}.
   *
   ...
   * methods.
   */
  _normalize(nodes: NewNodeForInterpolation): (Expression | string)[] {
  ...
  /** Like {@link normalize}, but also flattens a list of nodes. */
  _normalizeList(nodes: NewNodeForInterpolation[]): (Expression | string)[] {
    const result: Array<string | Expression> = [];
