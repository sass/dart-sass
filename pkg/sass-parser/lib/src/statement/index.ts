...
/** A copy of the internal PostCSS type of the same name. */
interface ContainerWithChildren<Child extends postcss.Node = postcss.ChildNode>
  extends postcss.Container<Child> {
  nodes: Child[];
}

/** A {@link Statement} that has actual child nodes. */
export type StatementWithChildren = ContainerWithChildren<ChildNode> &
  Statement;
