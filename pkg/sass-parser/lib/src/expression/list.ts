...
export interface ListExpressionRaws {
   ...
   *
   * For zero-element lists that contain a comma, this is the whitespace between
   * the opening bracket and the comma. For zero-element lists that don't
   * contain a comma, this is the whitespace between the brackets.
   *
   ...
   *
   * This is only set automatically for lists with trailing commas, or for
   * zero-element lists that contain a comma.
   *
   * This is ignored for unbracketed lists with more than zero elements.
   */
  beforeClose?: string;
  ...
  implements Container<Expression, NewNodeForListExpression>
{
  ...
  toString(): string {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      result += (raw?.before ?? '') + element + (raw?.after ?? '');
    ...
    if (this.separator === ',' && this.raws.trailingComma) result += ',';
    if (this.brackets || this.nodes.length === 0) {
