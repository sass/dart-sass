...
describe('a map entry', () => {
  ...
  function describeNode(description: string, create: () => IfConditionSass): void {
    describe(description, () => {
      ...
      it('has an expression', () => expect(node).toHaveNode('expression', 'true', 'boolean'));
    });
  }
  ...
  describe('constructed manually', () => {
    describeNode('with an Expression', () => new IfConditionSass(new BooleanExpression({value: true})));

    describeNode('with ExpressionProps', () => new IfConditionSass({value: true}));

    describe('with an object', () => {
      describeNode(
        'with an Expression',
        () =>
          new IfConditionSass({expression: new BooleanExpression({value: true})}),
      );
  ...
  describe('clone()', () => {
    ...
    describe('with no overrides', () => {
      ...
      describe('has the same properties:', () => {
        it('expression', () => expect(clone).toHaveNode('expression', 'true', 'boolean'));
      });
    ...
    describe('overrides', () => {
      ...
      describe('expression', () => {
        it('defined', () =>
          expect(node.clone({expression: {text: 'baz'}})).toHaveStringExpression(
            'expression',
            'baz',
          ));

        it('undefined', () =>
          expect(node.clone({expression: undefined})).toHaveNode('expression', 'true', 'boolean'));
      });
    });
  });
